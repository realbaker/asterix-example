<?php
    ...
    $primary_btn = get_field('company_designation_hero_button');
  ?>
  <section class="hero group bg-blue-opacity" style="<?php if($hero_bg){echo 'background-image: url('.$hero_bg.')';} ?>">
    <div class="container">
      <div class="hero-content <?php if(!$hero_bg){echo 'full-title';} ?>">
        ...
        <?php if($primary_btn): ?>
          <div class="hero-ctas">
              <a href="<?php echo $primary_btn['company_designation_hero_button_link']; ?>" class="button orange-button"><?php echo $primary_btn['company_designation_hero_button_text']; ?></a>
          </div>
        <?php endif; ?>
      </div>
      <?php
      if( have_rows('company_designation_hero_benefits') ):
        echo '<div class="benefits-wrapper flex-col-wrapper">';
        while ( have_rows('company_designation_hero_benefits') ) : the_row(); 
          $benefit_icon = get_sub_field('company_designation_benefit_icon');
        ?>
          <div class="benefit-block five-column column text-center">
            <img src="<?php echo $benefit_icon['sizes']['thumbnail']; ?>" />
            <h4><?php the_sub_field('company_designation_benefit_title'); ?></h4>
            <p><?php the_sub_field('company_designation_benefit_text'); ?></p>
          </div>
      <?php
        endwhile;
        echo '</div>';
